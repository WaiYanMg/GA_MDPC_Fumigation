...
{
    public partial class frm_exposure_period : Form
    {
        public frm_exposure_period()
        {
            InitializeComponent();
        }
        ...
        string do_save_record()
        {
            ...
            Mst_ExpouserPeriod the_record = new Mst_ExpouserPeriod();
            ...
            try
            {
                ...
                if (record_id != "")
                {
                    the_record = (from c in dc.Mst_ExpouserPeriods where record_id == c.ExpouserPeriodID select c).FirstOrDefault();

                }
                else
                {
                    the_record = new Mst_ExpouserPeriod()
                    {
                        ...
                        ExpouserPeriodID = Guid.NewGuid().ToString(),
                        ModifiedOn = DateTime.Now,
                        ModifiedBy = Program.get_current_user_id()

                    };
                    dc.Mst_ExpouserPeriods.InsertOnSubmit(the_record);
                }
                ...
                the_record.ExpouserPeriod = tb_period.Text;
                ...
                {
                    record_id = the_record.ExpouserPeriodID;
        ...
        { // user access;
                      ...
                      DialogResult.Yes)
            {
                Linq_FumigationDataContext dc = new Linq_FumigationDataContext(Program.get_main_connection());
                Mst_ExpouserPeriod the_record = new Mst_ExpouserPeriod();
                the_record = (from c in dc.Mst_ExpouserPeriods where c.Active == true && c.ExpouserPeriodID == record_id select c).FirstOrDefault();
        ...
        void do_load()
        {
            ...
            Linq_FumigationDataContext dc = new Linq_FumigationDataContext(Program.get_main_connection());
            Mst_ExpouserPeriodView the_result = (from c in dc.Mst_ExpouserPeriodViews where c.Active == true && c.ExpouserPeriodID == record_id select c).FirstOrDefault();
            if (record_id != null || record_id == "")
            {
                tb_period.Text = the_result.ExpouserPeriod;
        ...
        void load_list()
        {
            ...
            List<Mst_ExpouserPeriodView> records = (from c in dc.Mst_ExpouserPeriodViews
                                                    where c.Active == true
                                                 && ((tb_search.Text == "") || (tb_search.Text != "" &&
                                                 (c.ExpouserPeriod.Contains(tb_search.Text) ||
        ...
        {
            Mst_ExpouserPeriodView the_result = (Mst_ExpouserPeriodView)gc_list.GetRow(gc_list.GetSelectedRows().FirstOrDefault());
            record_id = the_result.ExpouserPeriodID;
