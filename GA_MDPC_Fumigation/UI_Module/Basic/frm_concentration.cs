...
{
    public partial class frm_concentration : Form
    {
        public frm_concentration()
        {
            InitializeComponent();
        }
        ...
        string do_save_record()
        {
            ...
            Mst_Concentration the_record = new Mst_Concentration();
            ...
            try
            {
                ...
                if (record_id != "")
                {
                    the_record = (from c in dc.Mst_Concentrations where record_id == c.ConcentrationID select c).FirstOrDefault();

                }
                else
                {
                    the_record = new Mst_Concentration()
                    {
                        ...
                        ConcentrationID = Guid.NewGuid().ToString(),
                        ModifiedOn = DateTime.Now,
                        ModifiedBy = Program.get_current_user_id()

                    };
                    dc.Mst_Concentrations.InsertOnSubmit(the_record);
                }
                ...
                the_record.Concentration = tb_concentration.Text;
                ...
                {
                    record_id = the_record.ConcentrationID;
        ...
        { // user access;
                      ...
                      DialogResult.Yes)
            {
                Linq_FumigationDataContext dc = new Linq_FumigationDataContext(Program.get_main_connection());
                Mst_Concentration the_record = new Mst_Concentration();
                the_record = (from c in dc.Mst_Concentrations where c.Active == true && c.ConcentrationID == record_id select c).FirstOrDefault();
        ...
        void do_load()
        {
            ...
            Linq_FumigationDataContext dc = new Linq_FumigationDataContext(Program.get_main_connection());
            Mst_ConcentrationView the_result = (from c in dc.Mst_ConcentrationViews where c.Active == true && c.ConcentrationID == record_id select c).FirstOrDefault();
        ...
        void load_list()
        {
            ...
            List<Mst_ConcentrationView> records = (from c in dc.Mst_ConcentrationViews
                                                   where c.Active == true
                                                 && ((tb_search.Text == "") || (tb_search.Text != "" &&
                                                 (c.Concentration.Contains(tb_search.Text) ||
        ...
        {
            Mst_ConcentrationView the_result = (Mst_ConcentrationView)gc_list.GetRow(gc_list.GetSelectedRows().FirstOrDefault());
            record_id = the_result.ConcentrationID;
