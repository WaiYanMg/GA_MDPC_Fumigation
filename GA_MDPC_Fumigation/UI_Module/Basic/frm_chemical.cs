...
{
    public partial class frm_chemical : Form
    {
        public frm_chemical()
        {
            InitializeComponent();
        ...
        private void frm_chemical_Load(object sender, EventArgs e)
        {
        ...
        bool do_save_record_verify()
        {
            lbl_message.Text = "";
            bool is_error = false;
            if (tb_chemical_name.Text.Trim() == "")
            {
                is_error = true;
                tb_chemical_name.Focus();
                lbl_message.Text = "Need to type Chemical Name";
            }
        ...
        string do_save_record()
        {
            ...
            Mst_Chemical the_record = new Mst_Chemical();
            ...
            try
            {
                ...
                if (record_id != "")
                {
                    the_record = (from c in dc.Mst_Chemicals where record_id == c.ChemicalID select c).FirstOrDefault();

                }
                else
                {
                    the_record = new Mst_Chemical()
                    {
                        ...
                        ChemicalID = Guid.NewGuid().ToString(),
                        ModifiedOn = DateTime.Now,
                        ModifiedBy = Program.get_current_user_id()

                    };
                    dc.Mst_Chemicals.InsertOnSubmit(the_record);
                }
                ...
                the_record.ChemicalName = tb_chemical_name.Text;
                ...
                {
                    record_id = the_record.ChemicalID;
        ...
        { // user access;
                      ...
                      DialogResult.Yes)
            {
                Linq_FumigationDataContext dc = new Linq_FumigationDataContext(Program.get_main_connection());
                Mst_Chemical the_record = new Mst_Chemical();
                the_record = (from c in dc.Mst_Chemicals where c.Active == true && c.ChemicalID == record_id select c).FirstOrDefault();
        ...
        void do_load()
        {
            ...
            Linq_FumigationDataContext dc = new Linq_FumigationDataContext(Program.get_main_connection());
            Mst_ChemicalView the_result = (from c in dc.Mst_ChemicalViews where c.Active == true && c.ChemicalID == record_id select c).FirstOrDefault();
            if (record_id != null || record_id == "")
            {
                tb_chemical_name.Text = the_result.ChemicalName;
        ...
        void load_list()
        {
            ...
            List<Mst_ChemicalView> records = (from c in dc.Mst_ChemicalViews
                                              where c.Active == true
                                                 && ((tb_search.Text == "") || (tb_search.Text != "" &&
                                                 (c.ChemicalName.Contains(tb_search.Text) ||
        ...
        {
            Mst_ChemicalView the_result = (Mst_ChemicalView)gc_list.GetRow(gc_list.GetSelectedRows().FirstOrDefault());
            record_id = the_result.ChemicalID;
