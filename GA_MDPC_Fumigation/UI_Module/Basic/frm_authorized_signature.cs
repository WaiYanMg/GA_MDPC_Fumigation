...
{
    public partial class frm_authorized_signature : Form
    {
        public frm_authorized_signature()
        {
            InitializeComponent();
        }
        ...
        bool do_save_record_verify()
        {
            lbl_message.Text = "";
            bool is_error = false;
            if (tb_signature_name.Text.Trim() == "")
            {
                is_error = true;
                tb_signature_name.Focus();
                lbl_message.Text = "Need to type Authorized Signature Name";
            }
        ...
        string do_save_record()
        {
            ...
            Mst_AuthorizedSignature the_record = new Mst_AuthorizedSignature();
            ...
            try
            {
                ...
                if (record_id != "")
                {
                    the_record = (from c in dc.Mst_AuthorizedSignatures where record_id == c.SignatureID select c).FirstOrDefault();

                }
                else
                {
                    the_record = new Mst_AuthorizedSignature()
                    {
                    ...
                    dc.Mst_AuthorizedSignatures.InsertOnSubmit(the_record);
                }
                ...
                the_record.SignatureName = tb_signature_name.Text;
                ...
                {
                    record_id = the_record.SignatureID;
        ...
        { // user access;
                      ...
                      DialogResult.Yes)
            {
                Linq_FumigationDataContext dc = new Linq_FumigationDataContext(Program.get_main_connection());
                Mst_AuthorizedSignature the_record = new Mst_AuthorizedSignature();
                the_record = (from c in dc.Mst_AuthorizedSignatures where c.Active == true && c.SignatureID == record_id select c).FirstOrDefault();
        ...
        void do_load()
        {
            ...
            Linq_FumigationDataContext dc = new Linq_FumigationDataContext(Program.get_main_connection());
            Mst_AuthorizedSignatureView the_result = (from c in dc.Mst_AuthorizedSignatureViews where c.Active == true && c.SignatureID == record_id select c).FirstOrDefault();
            if (record_id != null || record_id == "")
            {
                tb_signature_name.Text = the_result.SignatureName;
        ...
        void load_list()
        {
            ...
            List<Mst_AuthorizedSignatureView> records = (from c in dc.Mst_AuthorizedSignatureViews
                                                         where c.Active == true
        ...
        {
            Mst_AuthorizedSignatureView the_result = (Mst_AuthorizedSignatureView)gc_list.GetRow(gc_list.GetSelectedRows().FirstOrDefault());
            record_id = the_result.SignatureID;
